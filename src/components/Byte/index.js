...
const Byte = ({
  ...
  if (image != null) {
    renderImage = (
      <div
        className="byte-image post-image animate-scanline"
  ...
  if (related != null) {
    ...
    relatedLink = (
      <div className="related-link">
        <h4 style={{ marginBottom: `0` }}>Related Article:</h4>{" "}
        <Link to={uri}> &gt; {related.title}</Link>
      </div>
    )
  } else {
    relatedLink = null
  ...
  if (purchaseLink != null) {
    purchaseBookLink = (
      <a href={purchaseLink} target="_blank" className="button purchase">
        Purchase this book
      </a>
    )
  } else {
