...
const BytesPage = () => {
 ...
 /_.___/\\__, /\\__/\\___/____/  
      ...
      <p style={{ textAlign: `center`, marginBottom: `2em` }}>
        BYTES are micro-reviews of our recommended books.
      </p>
      <div className="bytes-page">
        {new Array(totalBytes).fill("").map((element, i) => (
          <Byte
            ...
            author={
              bytes.allWpPost.edges[`${i}`].node.bytesFields.bytesBookAuthor
            }
            excerpt={bytes.allWpPost.edges[`${i}`].node.content}
            pub={bytes.allWpPost.edges[`${i}`].node.bytesFields.bytesDate}
            purchaseLink={
              bytes.allWpPost.edges[`${i}`].node.bytesFields.bytesPurchaseLink
            }
            related={
              bytes.allWpPost.edges[`${i}`].node.bytesFields.bytesRelatedArticle
            }
          />
        ))}
      </div>
